...
- **Addon version**: 20260409-06 (date-based versioning)
...
| Architecture | Site 1 (unauth) | Site 2 (auth) | NOP encoding |
|---|---|---|---|
| aarch64 | `0x167214` (8 bytes) | `0x167D20` (8 bytes) | `d503201f` (ARM64) |
| x86_64 | `0x114D1F` (16 bytes) | `0x11583D` (13 bytes) | `0x90` (x86) |

Patched libraries live in `unifi/rootfs/usr/lib/unifi/lib/native/Linux/{arch}/`
and overwrite the ones installed by the UniFi deb via `COPY rootfs /`.
...
bug may be fixed upstream. Original md5:
- aarch64: `27e786235fae4a052bc808c3c13dfc19`
- x86_64: `28d3a1a1dcb6c728ccf3bd613a5888d3`
...
and the modal always shows. Suppressed at startup via `init-unifi/run` by
patching the frontend JS: `sed -i 's/useState)(!s)/useState)(!1)/g'` on the
swai bundle.

## Upstream PR

Submitted as [hassio-addons/addon-unifi#631](https://github.com/hassio-addons/addon-unifi/pull/631).
Includes the version bump, Java 25, TURN patch (both architectures), and nag fix.
PR branch: `pr/unifi-10.2.105-turn-fix` in our fork.
...
- `unifi/rootfs/usr/lib/unifi/lib/native/Linux/aarch64/libubnt_webrtc_jni.so` - Patched WebRTC library (aarch64)
- `unifi/rootfs/usr/lib/unifi/lib/native/Linux/x86_64/libubnt_webrtc_jni.so` - Patched WebRTC library (x86_64)
